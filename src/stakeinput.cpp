...
bool CHotxStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CHotxStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CHotxStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CHotxStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CHotxStake::GetModifier(uint64_t& nStakeModifier)
{
...
CBlockIndex* CHotxStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;

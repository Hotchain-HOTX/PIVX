...
CzHOTXWallet::CzHOTXWallet(std::string strWalletFile)
{
...
bool CzHOTXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CzHOTXWallet::Lock()
{
    seedMaster = 0;
}

void CzHOTXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHOTXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzHOTXWallet::LoadMintPoolFromDB()
{
...
void CzHOTXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHOTXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHOTXWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzHOTXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
...
uint512 CzHOTXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHOTXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHOTXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzHOTXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

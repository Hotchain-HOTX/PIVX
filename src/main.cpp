...
#include "zhotx/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zHOTX spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zHOTX without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zHOTX spend with serial %s from tx %s is not in valid range\n", __func__,
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zhotx");
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZHOTXStake)
{
    ...
    if (nHeight <= 43200) {
        ...
        //When zHOTX is staked, masternode only gets 2 HOTX
        ret = 3 * COIN;
        if (isZHOTXStake)
            ret = 2 * COIN;
    }
...
void RecalculateZHOTXMinted()
{
    ...
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZHOTX..."), percent);
        }
...
void RecalculateZHOTXSpent()
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = (int)( (double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()) );
            uiInterface.ShowProgress(_("Recalculating spent ZHOTX..."), percent);
        }
...
bool RecalculateHOTXSupply(int nHeightStart)
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating HOTX supply..."), percent);
        }
...
bool UpdateZHOTXSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zHOTX supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateHOTXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zHOTX money supply in the block index
    if (!UpdateZHOTXSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zHOTX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zHotxSpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert HOTX to zHOTX

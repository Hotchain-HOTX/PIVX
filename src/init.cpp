...
#include "zhotx/accumulatorcheckpoints.h"
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "hotchain.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "hotchaind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the HOTX and zHOTX money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in HOTX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in HOTX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, hotchain, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in HOTX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all HOTCHAIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-hotxstake=<n>", strprintf(_("Enable or disable staking functionality for HOTX inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zHOTX spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zhotxbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zHOTX backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizehotchainamount=<n>     " + strprintf(_("Keep N HOTX anonymized (default: %u)"), 0) + "\n";
...
/** Sanity checks
 *  Ensure that HOTCHAIN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single HOTCHAIN process is using the data directory.
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HOTCHAIN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //HOTCHAIN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //HOTCHAIN specific: zerocoin and spork DB's
                ...
                // HOTCHAIN: load previous sessions sporks if we have them.
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight>0 && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    if (pblockindex->GetZerocoinSupply() < zhotxSupplyCheckpoint) {
                        ...
                        LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zhotxSupplyCheckpoint/COIN);
                        reindexDueWrappedSerials = true;
                    } else if (pblockindex->GetZerocoinSupply() > zhotxSupplyCheckpoint) {
                        // Trigger global zHOTX reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateHOTXSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight>0 && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zhotxSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                    if (pblockindex->GetZerocoinSupply() != zhotxSupplyCheckpoint)
                        return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zhotxSupplyCheckpoint/COIN));
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzHOTXWallet(pwalletMain->strWalletFile);
...
//    }
//
//    nAnonymizeHotxAmount = GetArg("-anonymizehotchainamount", 0);
//    if (nAnonymizeHotxAmount > 999999) nAnonymizeHotxAmount = 999999;
//    if (nAnonymizeHotxAmount < 2) nAnonymizeHotxAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1HOTX+1000 == (.1HOTX+100)*10
       10HOTX+10000 == (1HOTX+1000)*10
    */
    ...
    */
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", true)) {
            // Run a thread to precompute any zHOTX spends

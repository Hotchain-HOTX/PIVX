...
void ZHotxControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zhotxTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zhotxTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zHOTX seed used to mint this zHOTX is not the same as currently hold in the wallet";
            else
